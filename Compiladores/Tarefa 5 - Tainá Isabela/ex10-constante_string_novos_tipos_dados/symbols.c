...
type_symbol_table_string_entry *sym_string_declare(char *s) {
    ...
    type_symbol_table_string_entry *str = sym_string_find(s);

    if (str != NULL) {
        printf("[ERRO] String ja declarada.\n");
        return NULL;
    }
    ...
    if ( symbol_table_string.n_strings < MAX_SYMBOLS ) {
        ...
        valors = strcpy(symbol_table_string.string[n_str].value, s);
        names = strcpy(symbol_table_string.string[n_str].name, str_name);
        if (valors == NULL || names == NULL) {
            printf("[ERRO] Nao foi possivel alocar memoria para a string.\n");
            return NULL;
        } else {
...
void initSymbolTableString() {
    ...
    // 
    for (i = 0; i < MAX_SYMBOLS; i++) {
        strcpy(symbol_table_string.string[i].name, "0");
