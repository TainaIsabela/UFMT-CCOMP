...
int declarationV(char *var_name, int var_type) {
    ...
    int ok1 = match(SEMICOLON);
    if ( ok1 != true ) {
        printf("[ERRO] Deveria existir um ';'.\n");
        return false;
    } else {
        return true;
    }
...
int declarationF(char *func_name, int func_type) {
    ...
    if ( match(OPEN_PAR) ) {
        int end = false;
        while ( (end != true) && (nparams < MAX_PARAMS) ) {
            int var_type;
    ...
    if (lookahead->tag == CLOSE_PAR) {
        ...
        if (ok1 == true && ok2 == true) {
            sym_func_declare(func_name, func_type, nparams, params);
            return true;
        } else {
            printf("[ERRO] Esperado ';' apos a declaracao de funcao.\n");
            return false;
        }
    }
    return false;
...
int statement (void) {
    ...
    if (lookahead->tag == READ) {
        ...
        if ( (lookahead->tag == ID) || (lookahead->tag == NUM) ) {
            match(lookahead->tag);
            if ( match(lookahead->tag) ) {
                if ( (lookahead->tag == ID) || (lookahead->tag == NUM) ) {
                    match(lookahead->tag);
                    match(SEMICOLON);
                    gen_assing(lookahead->lexema, lookahead->tag);
                    return true;
                }
            }
        }
 ...
 */
////// TODO
int B() { // Fazer ele verificar para todos os casos possiveis 
    int operator;
