var cart = {
  ...
  init: () => {
    ...
    for (let id in products) {
      ...
      item.querySelector(".p-price").textContent = "R$" + p.price.toFixed(2);
  ...
  list: () => {
    ...
    if (empty) {
      ...
      item = document.getElementById("template-cart-saida").content.cloneNode(true);
      cart.hItems.appendChild(item);
    }
  },
  add: (id) => {
  ...
  change: (pid, qty) => {
    if (qty <= 0) {
      ...
      for (let id in cart.items) {
        ...
        document.getElementById("c-total").innerHTML = "TOTAL: R$" + total;
      }
    }
  },
